...
dt <- melt(x, id.vars = "feature.long", measure.vars = c("base", "pos", "neg"), variable.name = "subset", value.name = "log.odds")
dt <- rbindlist(list(dt, data.table(feature.long = "Total", subset = "total", log.odds = dt[subset == "pos", sum(log.odds)] - dt[subset == "neg", sum(log.odds)])), use.names = T, fill = T)
dt[, subset := factor(subset, levels = c("pos", "neg", "base", "total"), ordered = T)]

p <- ggplot(data = dt, aes(x = feature.long, y = log.odds, fill = subset, label = sprintf("%.2f", log.odds))) + geom_col() + theme_classic() + scale_fill_manual(values=c("#009E73", "#D55E00", "#FFFFFF", "#A9A9A9")) +
  theme(legend.position = "none", axis.text.x = element_text(angle = 60, hjust = 1, size = 18), axis.text.y = element_text(size = 18), axis.title = element_text(size = 20), 
        plot.title = element_text(size = 18, hjust = 0.5)) +
  ggtitle(paste0("Top 20 important features for ", gene, " association with Alzheimer's Disease")) + xlab("Feature") + ylab("Log odds") +
  geom_label(aes(label = ifelse(log.odds > 0 & subset != "base", ifelse(subset == "neg", round(-1*log.odds, 2), round(log.odds, 2)), NA)), position = position_stack(vjust = 0.5))
